...
fun DetallesRuta(
    ...
    LaunchedEffect(ubicacionActualizada) {
        ubicacionActualizada?.firstOrNull()?.let { nuevaUbicacion ->
            if(esConductor.value) {
                ...
                //NOTA: Lo comento porque el cliente no debería moverse. Una vez se crea la ruta el conductor
                // debe tener claro donde va, sin cambios
    ...
    rutaActiva?.let { ruta ->
        NonAppScaffold(
            navController = navController,
            showBack = rutaViewModel.puedeVolver || ((user!!.id == ruta.conductor || user!!.id == ruta.cliente) && ruta.finalizado),
            onBackClick = {
            ...
            Text(
                text = textoTitulo,
                style = MaterialTheme.typography.headlineSmall, //TODO Cambiar por algo con menos padding
            ...
            /**
             * La ruta se ha cancelado
             */
            if(ruta.finalizado && ruta.cancelada) {
                Row(
                    verticalAlignment = Alignment.CenterVertically,
                    modifier = Modifier.fillMaxWidth()
                ) {
                    Icon(
                        imageVector = Icons.Default.Close, // icono de llegada
                        contentDescription = stringResource(R.string.rutaHaSidoCancelada),
                        tint = Color(0xFFFF9800), // naranja
                        modifier = Modifier.size(32.dp)
                    )
                    Spacer(Modifier.width(8.dp))
                    Text(
                        text = stringResource(R.string.rutaHaSidoCancelada),
                        style = MaterialTheme.typography.bodyLarge
                    )
                }
            }
